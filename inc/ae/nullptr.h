 ...
 * (`AE_PTR_T_MIN`), если код компилируется в языке C, где ключевое слово `nullptr` не
 * поддерживается.
 *
 ...
 *
 * Макросы:
 * - `nullptr` — определяется как `AE_PTR_T_MIN` в случае компиляции на C,
 ...
 *
 * Этот макрос определяет `nullptr` как минимальное значение указателя (`AE_PTR_T_MIN`),
