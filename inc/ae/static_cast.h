 ...
 *
 * Этот файл содержит определение макроса `ae_static_cast`,
 ...
 *
 * @note Макрос `ae_static_cast` удобен для использования в проектах,
 *       которые могут быть скомпилированы как компилятором C++,
 *       так и компилятором C.
 */

#ifndef AE_STATIC_CAST_H
#define AE_STATIC_CAST_H

/**
 * @def ae_static_cast(T, V)
 ...
 * @code{.cpp}
 * int a = 5;
 * double b = ae_static_cast(double, a); // В C++ это приведет тип int к double
 * @endcode
 */
#ifndef __cplusplus
#    define ae_static_cast(T, V) ((T)V) ///< В языке C используется приведение типов через (T)V
#else
#    define ae_static_cast(T, V) static_cast<T>(V) ///< В C++ используется оператор static_cast
#endif

#endif // AE_STATIC_CAST_H
