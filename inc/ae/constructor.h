/**
 * @file constructor.h
 * @brief Определение макроса `ae_constructor` для различных компиляторов.
 *
 * Этот файл содержит определение макроса `ae_constructor`, который позволяет
 * объявлять функции-конструкторы, выполняемые перед вызовом функции `main()`.
 * Макрос адаптирован для работы с компиляторами GCC, Clang и MSVC, а также
 * предоставляет пустое определение для неподдерживаемых компиляторов.
 *
 * @details
 ...
 */
#    define ae_constructor(f) __attribute__((constructor)) static void f(void)
