...
/**
 * @brief Сбрасывает код ошибки времени выполнения.
 *
 * Эта функция устанавливает значение переменной `m_runtime_error_code`
 * на переданный код ошибки. Это может быть полезно для обновления
 * состояния ошибки в процессе выполнения программы.
 *
 * @param error_code Код ошибки, который будет установлен в качестве
 *                   текущего кода ошибки времени выполнения.
 *
 * @note После вызова этой функции, текущее значение кода ошибки
 *       будет заменено на переданное значение.
 *
 * @see m_runtime_error_code
 * @see ae_error_code_t
 */
...
/**
 * @brief Очищает код ошибки времени выполнения и возвращает предыдущее значение.
 *
 * Эта функция сбрасывает текущий код ошибки времени выполнения на
 * значение `AE_RUNTIME_ERROR_OK` и возвращает предыдущее значение
 * кода ошибки. Это может быть полезно для сброса состояния ошибки
 * в процессе выполнения программы и для сохранения информации о
 * предыдущей ошибке.
 *
 * @return Код ошибки времени выполнения, который был установлен до
 *         вызова этой функции, представлен типом `ae_error_code_t`.
 *
 * @see ae_runtime_error_code
