...
/**
 * @brief Проверяет, является ли индекс допустимым для блока памяти.
 *
 * Эта функция проверяет, находится ли указанный индекс в пределах
 * допустимого диапазона для блока памяти. Она использует функцию
 * @ref ae_memory_block_size для получения общего размера блока памяти
 * и сравнивает индекс с этим размером.
 *
 * @param self Указатель на структуру @ref ae_memory_block,
 *             для которой необходимо проверить индекс.
 * @param index Индекс, который нужно проверить на допустимость.
 *
 * @return true, если индекс допустим (меньше размера блока памяти),
