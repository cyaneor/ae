...
/**
 * @brief Копирует данные между двумя диапазонами памяти с учётом выравнивания.
 *
 * Эта функция выполняет поблочное копирование данных между двумя диапазонами памяти,
 * начиная с начала и продолжая до достижения конца одного из диапазонов.
 *
 * Копирование выполняется с учётом выравнивания указателей
 * для различных типов данных (128-бит, 64-бит, 32-бит, 16-бит и 8-бит).
 *
 * - Функция проверяет выравнивание указателей для разных типов данных
 *   (например, для 128-битных или 64-битных элементов)
 *   и выбирает соответствующий способ копирования.
 * - В зависимости от выравнивания указателей, копирование может быть выполнено
 *   для 128-битных, 64-битных, 32-битных, 16-битных или 8-битных элементов.
 * - Если указатели не выровнены, копирование выполняется поблочно по 8-битным элементам.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 ...
 *
 * @return Указатель на следующий элемент в целевом диапазоне после завершения копирования.
 *         Если копирование завершено, указатель может быть равен `dst_end`.
 *
 * @note Эта функция может использовать различные вспомогательные функции,
 *       такие как `ae_memory_raw_copy_u128`, `ae_memory_raw_copy_u64`,
 *       и другие в зависимости от выравнивания.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
...
#ifdef AE_U128_T_SIZE
/**
 * @brief Копирует данные между двумя диапазонами памяти в обратном порядке для 128-битных
 * элементов.
 *
...
/**
 * @brief Перемещает данные между двумя диапазонами памяти с учетом выравнивания.
 *
 * Эта функция выполняет перемещение данных между двумя диапазонами памяти с учётом выравнивания
 * указателей для различных типов данных (128-бит, 64-бит, 32-бит, 16-бит и 8-бит). Перемещение
 * выполняется в зависимости от выравнивания указателей. Функция поддерживает обработку случая
 * перекрытия диапазонов памяти и использует соответствующий алгоритм перемещения данных
 * для каждого типа данных.
 *
 * - Функция проверяет выравнивание указателей для различных типов данных
 *   и выбирает соответствующий способ перемещения: 128-битные, 64-битные,
 *   32-битные, 16-битные или 8-битные элементы.
 * - Если указатели не выровнены для какого-либо из типов,
 *   перемещение выполняется поблочно по 8-битным элементам.
 * - Функция может использовать различные вспомогательные функции,
 *   такие как `ae_memory_raw_move_u128`, `ae_memory_raw_move_u64` и другие.
 * - Если диапазоны памяти перекрываются, используются соответствующие методы
 ...
 *         Это может быть равно `dst_end`, если перемещение завершено.
 *
 * @note Эта функция использует проверку выравнивания указателей
 *       и различные методы перемещения данных в зависимости от выравнивания.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
