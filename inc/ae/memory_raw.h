...
/**
 * @brief Функция повторного заполнения памяти.
 *
 * Данная функция выполняет перемещение данных из исходной области памяти
 * в целевую область памяти несколько раз, пока не будет достигнут размер,
 * указанный параметром `size`.
 *
 * @param dst Указатель на целевую область памяти,
 *            в которую будет происходить заполнение.
 * @param dst_end Указатель на конец целевой области памяти.
 * @param src Указатель на исходную область памяти,
 *            из которой будут копироваться данные.
 * @param src_end Указатель на конец исходной области памяти.
 * @param size Количество итераций для перемещения данных,
 *             также определяет общее количество повторений.
 *
 * @return Указатель на обновленную целевую область памяти.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
 ...
 *
 * Эта функция выполняет поиск указанного значения в диапазоне памяти, определенном указателями
 * на начало и конец, но в обратном направлении. Если значение найдено, возвращается указатель
 * на его последнее вхождение в диапазоне. Если значение не найдено, возвращается `nullptr`.
 *
 * @param begin Указатель на начало диапазона памяти,
 *              в котором будет выполняться поиск.
 * @param end Указатель на конец диапазона памяти.
