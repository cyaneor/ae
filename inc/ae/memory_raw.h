 ...
 *
 * Эта функция выполняет поблочное копирование данных из одного диапазона памяти
 * в другой в прямом порядке, используя инструкции SSE3
 * для эффективной обработки 128-битных данных.
 *
 * Копирование происходит по 128-битным блокам,
 * начиная с начала исходного диапазона и заканчивая на конце целевого диапазона.
 *
 * - Если оба указателя `dst` и `src` не равны `NULL`, функция копирует данные из диапазона,
 *   определенного указателями на начало исходного диапазона и конец целевого диапазона.
 * - Копирование происходит поблочно по 128-битным элементам в прямом порядке,
...
/**
 * @brief Копирует данные из одного диапазона памяти в другой для 128-битных элементов.
 *
 * Эта функция выполняет поблочное копирование данных из одного диапазона памяти в другой,
 * начиная с начала и продолжая до достижения конца одного из диапазонов. Копирование происходит
 * по 128-битным элементам.
 *
 * - Если оба указателя `dst` и `src` не равны `NULL`, функция копирует данные из диапазона,
 *   определенного указателями на начало и конец диапазона исходных данных, в диапазон назначения.
 * - Копирование происходит поблочно по 128-битным элементам, пока не будет достигнут конец одного
 *   из диапазонов.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
...
/**
 * @brief Копирует данные между двумя диапазонами памяти
 *        с учетом выравнивания и размера элементов.
 *
 * Эта функция выполняет копирование данных между двумя диапазонами памяти в зависимости
 * от их выравнивания и доступных оптимизаций, таких как SSE3 или другие типы данных,
 * такие как `u128`, `u64`, `u32`, и `u16`.
 *
 * Она автоматически выбирает наиболее эффективный способ копирования
 * в зависимости от доступных инструкций процессора
 * и выравнивания указанных диапазонов памяти.
 *
 * В зависимости от настроек компиляции и выравнивания,
 * функция может использовать различные подходы:
 * - Если доступна оптимизация SSE3,
 *   используется функция копирования с SSE3.
 * - Если доступен тип данных `u128`,
 *   используется функция копирования с этим типом.
 * - Если выравнивание позволяет,
 *   используется более эффективное копирование по 64-битным, 32-битным или 16-битным блокам.
 * - Если ни одно из этих условий не выполняется,
 *   данные копируются поблочно по 8-битным элементам.
 *
 * Функция проверяет выравнивание памяти
 * для каждого типа данных и применяет соответствующий метод копирования.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 ...
 *         в целевом диапазоне после завершения копирования.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
 ...
 *
 * Эта функция выполняет поблочное копирование данных из одного диапазона памяти в другой
 * в обратном порядке, используя инструкции SSE3 для эффективной обработки 128-битных данных.
 *
 * Копирование происходит по 128-битным блокам, начиная
 * с конца исходного диапазона и заканчивая на начале целевого диапазона.
 *
 * - Если оба указателя `dst` и `src` не равны `NULL`, функция копирует данные из диапазона,
 *   определенного указателями на конец исходного диапазона и начало целевого диапазона.
 * - Копирование происходит поблочно по 128-битным элементам в обратном порядке,
 *   пока не будет достигнут начало одного из диапазонов.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 ...
 *
 * @return Указатель на следующий элемент в целевом диапазоне после завершения копирования.
 *         Это может быть равно `dst`, если копирование завершено.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
...
/**
 * @brief Копирует данные между двумя диапазонами памяти
 *        в обратном порядке для 128-битных элементов.
 *
 * Эта функция выполняет поблочное копирование данных
 * из одного диапазона памяти в другой в обратном порядке.
 *
 * Копирование происходит по 128-битным элементам,
 * начиная с конца исходного диапазона и заканчивая на начале целевого диапазона.
 *
 * - Если оба указателя `dst` и `src` не равны `NULL`, функция копирует данные из диапазона,
 *   определенного указателями на конец исходного диапазона и начало целевого диапазона.
 * - Копирование происходит поблочно по 128-битным элементам в обратном порядке,
...
/**
 * @brief Копирует данные между двумя диапазонами памяти
 *        в обратном порядке с учетом выравнивания и размера элементов.
 *
 * Эта функция выполняет копирование данных между двумя диапазонами памяти в обратном порядке,
 * автоматически выбирая наиболее эффективный способ копирования в зависимости от выравнивания
 * и доступных оптимизаций.
 *
 * Она поддерживает различные методы копирования данных,
 * включая использование SSE3 и других типов данных,
 * таких как `u128`, `u64`, `u32`, и `u16`.
 *
 * Функция проверяет выравнивание памяти для каждого типа данных
 * и применяет соответствующий метод копирования в обратном порядке:
 * - Если доступна оптимизация SSE3,
 *   используется функция копирования с SSE3 в обратном порядке.
 * - Если доступен тип данных `u128`,
 *   используется функция копирования с этим типом в обратном порядке.
 * - Если выравнивание позволяет,
 *   используется более эффективное копирование по 64-битным, 32-битным или 16-битным блокам.
 * - Если ни одно из этих условий не выполняется,
 *   данные копируются поблочно по 8-битным элементам в обратном порядке.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 ...
 *
 * Эта функция выполняет перемещение данных между двумя диапазонами памяти,
 * начиная с `src` и заканчивая на `dst`, с учетом возможного перекрытия диапазонов.
 *
 * - Если диапазоны памяти перекрываются,
 *   сначала выполняется копирование данных в обратном порядке,
 *   чтобы избежать повреждения данных, а затем данные перемещаются в целевой диапазон.
 * - В случае отсутствия перекрытия данных используется стандартное копирование с SSE3.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 ...
 *
 * @return Указатель на следующий элемент
 *         в целевом диапазоне после завершения перемещения.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
...
#ifdef AE_U128_T_SIZE
/**
 * @brief Перемещает данные между двумя диапазонами памяти
 *        для 128-битных элементов с учетом перекрытия.
 *
 * Эта функция выполняет перемещение данных из одного диапазона памяти в другой для 128-битных
 * элементов. Если диапазоны памяти перекрываются, данные копируются в обратном порядке для
 * предотвращения потери информации, иначе используется стандартное копирование данных.
 *
 * - Если диапазоны памяти не перекрываются,
 *   функция использует копирование данных по 128-битным элементам.
 * - Если диапазоны памяти перекрываются,
 *   данные копируются в обратном порядке
 *   с использованием функции `ae_memory_raw_copy_rev_u128`.
 * - После копирования функция корректирует указатель,
 *   чтобы отразить новый конец целевого диапазона.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
 * @param src_end Указатель на конец исходного диапазона памяти.
 *
 * @return Указатель на следующий элемент в целевом диапазоне после завершения перемещения.
 *         Это может быть равно `dst_end`, если перемещение завершено.
 *         Если диапазоны перекрываются, возвращается скорректированный указатель.
 *
 * @note Эта функция использует проверку на перекрытие диапазонов с помощью
 *       `ae_ptr_range_is_overlapped` и выполняет соответствующее перемещение данных.
 *
 * @throw AE_RUNTIME_ERROR_NULL_POINTER
 *        Если @c dst или @c src является NULL.
 */
 ...
 *        с учетом выравнивания и размера элементов.
 *
 * Эта функция выполняет перемещение данных между двумя диапазонами памяти в зависимости от их
 * выравнивания и доступных оптимизаций, таких как SSE3 или другие типы данных, такие как `u128`,
 * `u64`, `u32`, и `u16`. Она автоматически выбирает наиболее эффективный способ перемещения в
 * зависимости от выравнивания указанных диапазонов.
 *
 * Функция проверяет выравнивание памяти для каждого типа данных
 * и применяет соответствующий метод перемещения:
 * - Если доступна оптимизация SSE3,
 *   используется функция перемещения с SSE3.
 * - Если доступен тип данных `u128`,
 *   используется функция перемещения с этим типом.
 * - Если выравнивание позволяет,
 *   используется более эффективное перемещение по 64-битным, 32-битным или 16-битным блокам.
 * - Если ни одно из этих условий не выполняется,
 *   данные перемещаются поблочно по 8-битным элементам.
 *
 * @param dst Указатель на начало целевого диапазона памяти.
 * @param dst_end Указатель на конец целевого диапазона памяти.
 * @param src Указатель на начало исходного диапазона памяти.
