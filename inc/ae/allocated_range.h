 ...
 *
 * @note Функция вызывает ae_runtime_allocator_free для освобождения памяти,
 *       а затем вызывает ae_memory_range_clear для очистки диапазона.
 */
AE_ATTRIBUTE(SYMBOL)
void
 ...
 * и повторно выбрасывает его после завершения обмена.
 *
 ...
 *       безопасности операций с памятью. Убедитесь, что оба указателя
 *       указывают на корректные и инициализированные диапазоны памяти.
 */
AE_ATTRIBUTE(SYMBOL)
void
