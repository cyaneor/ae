 ...
 *
 * Этот файл содержит макрос `ae_destructor`, который позволяет
 * определять функции деструкторов для различных компиляторов, таких как
 * GCC, Clang и MSVC. Деструкторы — это функции, которые автоматически
 * вызываются после выхода из функции `main()`, что позволяет выполнять
 * завершающие операции, такие как освобождение ресурсов.
 *
 *
 * @details В зависимости от типа компилятора,
 *          макрос `ae_destructor` будет определен различным образом:
 ...
 *
 * Основные функции:
 * - `ae_destructor(f)` - определяет функцию деструктора для соответствующего компилятора.
 *
 * @warning Если компилятор не поддерживает деструкторы,
 *          будет выдано предупреждение на этапе компиляции.
 */

#ifndef AE_DESTRUCTOR_H
#define AE_DESTRUCTOR_H
 ...
 */
#    define ae_destructor(f) __attribute__((destructor)) static void f(void)
