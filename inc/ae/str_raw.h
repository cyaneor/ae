...
AE_ATTRIBUTE(SYMBOL)
const ae_str_raw_t
ae_str_raw_find_value_ex(const ae_str_raw_t str, ae_usize_t str_len, ae_char_t value);
 ...
 * Для выполнения операции она использует функцию @ref ae_memory_append_ex,
 ...
 * затем выполняется сдвиг с помощью функции @ref ae_str_raw_shift_left_ex.
 *
 * @param str Указатель на строку, которая будет сдвинута.
 * @param shift Количество позиций, на которое необходимо сдвигать строку влево.
 *
 ...
 * Сдвиг выполняется с помощью функции @ref ae_str_raw_shift_left_ex.
 *
 ...
 *
 * Эта функция использует функцию @ref ae_str_raw_trim_left_ex для удаления всех пробельных
 ...
 *
 * Символы, которые подлежат удалению, определяются в статическом массиве.
