 ...
 *
 * Этот макрос позволяет инициализировать диапазон памяти, задавая
 * начальную и конечную точки, а также произвольное количество дополнительных
 * аргументов. Это может быть полезно для настройки объектов или структур,
 * которые требуют указания диапазона памяти и дополнительных параметров.
 *
 * @param begin Начальная точка диапазона памяти.
 * @param end Конечная точка диапазона памяти.
 * @param ... Дополнительные аргументы инициализации,
 *            которые будут переданы в макрос ae_initializer.
 *
 * @note Убедитесь, что переданные значения begin и end корректны и что
 *       они соответствуют ожидаемым типам. Также будьте внимательны при
 *       использовании дополнительных аргументов, так как они передаются
 *       в макрос ae_initializer.
 *
 ...
 * @endcode
 */
#define ae_memory_range_initializer(begin, end, ...) ae_initializer(begin, end, __VA_ARGS__)
