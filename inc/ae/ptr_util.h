 ...
 * `begin`, а затем вычитает это значение из разницы между концом диапазона `end` и началом
 * диапазона `begin`. Результатом является смещение указателя относительно конца диапазона.
 *
 * @param begin Указатель на начало диапазона.
 * @param end Указатель на конец диапазона.
 * @param ptr Указатель, смещение которого относительно диапазона нужно вычислить.
 *
 * @return Смещение указателя `ptr` относительно конца диапазона, выраженное в элементах.
 *
 * @note Этот макрос предполагает, что указатели `begin`, `end`,
 *       и `ptr` принадлежат одному и тому же массиву или диапазону.
 */
#define ae_ptr_mirror_from_end(begin, end, ptr) ((end - (ptr - begin)))
 ...
 */
#define ae_ptr_add_offset(ptr, offset) ae_ptr_from_addr(void, (ae_ptr_to_addr(ptr) + offset))
 ...
 */
#define ae_ptr_sub_offset(ptr, offset) ae_ptr_from_addr(void, (ae_ptr_to_addr(ptr) - offset))
