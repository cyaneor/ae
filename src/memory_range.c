...
{
    const void *end = ae_memory_range_cend(self);
...
{
    const void *begin = ae_memory_range_cbegin(self);
    const void *end   = ae_memory_range_cend(self);
...
{
    AE_RUNTIME_ASSERT(ae_ptr_range_validate(begin, end),
                      AE_RUNTIME_ERROR_INVALID_MEMORY_RANGE,
                      ae_memory_range_make_empty())

    return ae_memory_range_make(begin, end);
}

void
ae_memory_range_reset_with_validate(ae_memory_range_t *self, void *begin, void *end)
{
    ae_memory_range_t other = ae_memory_range_make(begin, end);
...
{
    void *_begin = ae_memory_range_begin(self);
    void *_end   = ae_memory_range_end(self);

    return ae_memory_range_make(_begin, _end);
}
...
ae_memory_range_swap(ae_memory_range_t *self, ae_memory_range_t *other)
{
    ae_runtime_try
    {
        ae_memory_range_t _t = ae_memory_range_shallow_copy(self);
        ae_memory_range_assign(self, other);
...
ae_memory_range_contain_range(const ae_memory_range_t *self,
                              ...
                              bool                     inclusive)
{
    return ae_memory_range_contain_ptr(self, begin, inclusive) &&
           ae_memory_range_contain_ptr(self, end, inclusive);
}
...
{
    const void *begin = ae_memory_range_cbegin(self);
    return ae_ptr_add(begin, index);
}

void *
ae_memory_range_at(const ae_memory_range_t *self, ae_usize_t index)
{
    void *ptr = ae_memory_range_at_unsafe(self, index);
    AE_RUNTIME_ASSERT(
        ae_memory_range_contain_ptr(self, ptr, false), AE_RUNTIME_ERROR_OUT_OF_RANGE, nullptr)
...
ae_memory_range_insert_range(ae_memory_range_t *self,
                             ...
                             const void        *end)
{
    void       *_begin = ae_memory_range_at(self, position);
    const void *_end   = ae_memory_range_end(self);
    ae_memory_raw_copy(_begin, _end, begin, end);
}
...
{
    const void *_begin = ae_memory_range_cbegin(other);
    const void *_end   = ae_memory_range_cend(other);

    ae_memory_range_insert_range(self, position, _begin, _end);
}
...
{
    AE_RUNTIME_ASSERT(ae_memory_range_check(other), AE_RUNTIME_ERROR_INVALID_MEMORY_RANGE)
    ae_memory_range_insert_other(self, position, other);
}
