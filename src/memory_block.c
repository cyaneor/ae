...
ae_memory_block_element_base_address(const struct ae_memory_block *self, const void *ptr)
{
    ...
    const void *begin = ae_memory_range_get_begin(ae_ptr_cast(const ae_memory_range_t, self));
    ...
    return ae_ptr_add_offset(begin, element_offset);
}
...
ae_memory_block_is_equal(const ae_memory_block_t *self, const ae_memory_block_t *other)
{
    return ae_memory_block_is_element_size_equal(self, other) &&
           ae_memory_range_is_equal(ae_ptr_cast(const ae_memory_range_t, self),
