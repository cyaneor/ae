...
ae_memory_raw_copy_backward(const void *dst, void *dst_end, const void *src, const void *src_end)
{
    if (ae_ptr_range_is_aligned_both(dst, dst_end, src, src_end, AE_U64_T_SIZE))
    {
        return ae_memory_raw_copy_backward_u64(dst, dst_end, src, src_end);
    }
    elif (ae_ptr_range_is_aligned_both(dst, dst_end, src, src_end, AE_U32_T_SIZE))
    {
        return ae_memory_raw_copy_backward_u32(dst, dst_end, src, src_end);
    }
    elif (ae_ptr_range_is_aligned_both(dst, dst_end, src, src_end, AE_U16_T_SIZE))
    {
        return ae_memory_raw_copy_backward_u16(dst, dst_end, src, src_end);
    }
    return ae_memory_raw_copy_backward_u8(dst, dst_end, src, src_end);
}
...
{
    return ae_memory_raw_find_u8(begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u8_t)));
}

const ae_u16_t *
ae_memory_raw_find_value_u16(const ae_u16_t *begin, const ae_u16_t *end, ae_u16_t value)
{
    return ae_memory_raw_find_u16(begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u16_t)));
}

const ae_u32_t *
ae_memory_raw_find_value_u32(const ae_u32_t *begin, const ae_u32_t *end, ae_u32_t value)
{
    return ae_memory_raw_find_u32(begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u32_t)));
}
...
{
    return ae_memory_raw_find_backward_u8(
        begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u8_t)));
}

const ae_u16_t *
ae_memory_raw_find_backward_value_u16(const ae_u16_t *begin, const ae_u16_t *end, ae_u16_t value)
{
    return ae_memory_raw_find_backward_u16(
        begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u16_t)));
}

const ae_u32_t *
ae_memory_raw_find_backward_value_u32(const ae_u32_t *begin, const ae_u32_t *end, ae_u32_t value)
{
    return ae_memory_raw_find_backward_u32(
        begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u32_t)));
}

const ae_u64_t *
ae_memory_raw_find_backward_value_u64(const ae_u64_t *begin, const ae_u64_t *end, ae_u64_t value)
{
    return ae_memory_raw_find_backward_u64(
        begin, end, &value, ae_ptr_add_offset(&value, sizeof(ae_u64_t)));
}
