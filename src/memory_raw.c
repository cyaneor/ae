...
{
    ae_runtime_assert(begin && end, AE_RUNTIME_ERROR_NULL_POINTER);

    ae_u8_t       *_begin = ae_ptr_cast(ae_u8_t, begin);
    const ae_u8_t *_end   = ae_ptr_cast(const ae_u8_t, end);

    while (_begin < _end)
    {
        const ae_usize_t len = ae_ptr_diff(_end, _begin);

#if defined(AE_COMPILE_OPTION_AVX512)
        if (len >= 64)
        {
            __m512i zmm0 = _mm512_set1_epi8(value);
            _mm512_storeu_si512(ae_ptr_cast(__m512i, _begin), zmm0);
            _begin += 64;
            continue;
        }
#endif
        ...
        if (len >= 32)
        {
            __m256i ymm0 = _mm256_set1_epi8(value);
            _mm256_storeu_si256(ae_ptr_cast(__m256i, _begin), ymm0);
            _begin += 32;
            continue;
        }
#endif
        ...
        if (len >= 16)
        {
            ...
            _mm_storeu_si128(ae_ptr_cast(__m128i, _begin), xmm0);
            _begin += 16;
            continue;
        }
#endif
...
{
    const void *_ptr = ae_ptr_add_offset(void, dst, shift);
...
{
    void       *_ptr     = ae_ptr_add_offset(void, dst, shift);
    const void *_src_end = ae_ptr_sub_offset(void, dst_end, shift);
                                   ...
                                   ae_char_t   value)
{
    ae_runtime_try
    {
        void       *_ptr     = ae_memory_raw_shift_right(dst, dst_end, shift);
        const void *_dst_end = ae_ptr_add_offset_unsafe(void, dst, shift);
